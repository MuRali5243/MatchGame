...
const failimg = 'https://assets.ccbp.in/frontend/react-js/match-game-trophy.png'
...
class Game extends Component {
  ...
  callTimer = () => {
    this.timerId = setInterval(() => {
      ...
      if (x >= 0) {
        ...
        this.setState(pre => ({fail: !pre.fail}))
      }
    }, 1000)
  }
  ...
  checkImage = id => {
    ...
    const imageObj = imagesList[image]
    if (id === imageObj.id) {
      const r = Math.floor(Math.random() * l)
      const s = score + 1
      this.setState({image: r, score: s})
    } else {
      this.setState(pre => ({fail: !pre.fail}))
    }
  }
  ...
  failed = () => {
    ...
    return (
      <div className="fail-cont">
        <img src={failimg} className="fail-image" alt="trophy" />
        <p>YOUR SCORE</p>
        <p>{score}</p>
  ...
  playAgain = () => {
    this.setState({score: 0, fail: false, time: 60}, this.callTimer())
  }

  render() {
    const {activeTab, image, fail, time, score} = this.state
    const imageObj = imagesList[image]
    ...
    return (
      <div className="main-cont">
        <ul className="head-cont">
          <li>
            <img className="website-logo" src={web} alt="website logo" />
          </li>
          <li className="score-timer">
            <p>{`Score: ${score}`}</p>
            <img className="timer-img" src={timer} alt="timer" />
            <p>{`${time} sec`}</p>
          </li>
        </ul>
        <div className="body-cont">
          {fail ? (
            ...
            <MatchCard
              ...
              tabsList={tabsList}
              imageObj={imageObj}
              componentWillUn={this.componentWillUnmount}
